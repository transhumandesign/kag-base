	...
	if (getNet().isServer())
	{
		...
		if (this.getMap().getBlobsInRadius(this.getPosition(), RAID_RADIUS, @blobsInRadius))
		{
			...
			if (attackersCount > 0 && (friendlyCount == 0 || !not_neutral))
			{
				...
				s32 captureTime = this.get_u32("capture time");
				...
				this.set_u32("capture time", captureTime);
...
void onRender(CSprite@ this)
{
	...
	if (isUnderRaid(blob))
	{
		...
		{
			const s32 captureTime = blob.get_u32("capture time");
