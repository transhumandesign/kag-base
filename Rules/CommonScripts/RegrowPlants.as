...
//dont set values lower than 0.001
//if you want to prevent a thing from growing at all, set chance to -1 rather than 0
...
void onInit(CRules@ this)
{
	...
	if (map !is null)
	{
		//add fake tiles to the start of TileInfo arrays so if findTileByCoords returns 0 it doesnt refer to a tile we should've updated
		...
		for (u32 x = 0; x < map.tilemapwidth; x++)
		{
			for (u32 y = 0; y < map.tilemapheight; y++)
			{
				...
				//add if tile above is empty or it's a castle/wood wall so grass grows after its destroyed
...
{
	//refill TileInfo arrays with info for newly loaded map
	dirt_tiles.clear();
	castle_tiles.clear();
	onInit(this);
}
...
void onSetTile(CMap@ this, u32 index, TileType newtile, TileType oldtile)
{
	...
	if (this.isTileGround(oldtile)) //dirt leaves dirt background after its destroyed, so no need to check for dirt tiles below it
	{
		dirt_tiles.removeAt(tindex);
	}
	if (castle_stuff.find(oldtile) != -1)
	{
		castle_tiles.removeAt(tindex); //got destroyed/mossified, remove from array
	}
...
{		
	if (getGameTime() % random_time == 0)
	{
				...
				if (random_grow <= bush_grow_chance)
				{
					...
					if (map.getBlobsInRadius(tinfo.coords, 8.0f, blobs))
					{
						for (int a = 0; a < blobs.size(); a++)
						{
							...
							if (blob.getName() == "bush") //check for bushes, dont grow if there's already one nearby
							{
								near_bush = true; 
								break;
							}
		...
		for (int i = 1; i < castle_tiles.size(); i++)
		{
			...
			{
				if (luck > 0 && random_grow - luck <= moss_stone_chance && tinfo.mossTime()) // check for luck being non-zero so stone structures get mossified from ground level
				{
